        ...
        if (
          ...
          fs.rename(req.file.path, file, async function (err) {
            if (err) {
              ...
              file =
                __dirname.replace(/(\s+)/g, "\\$1") +
                "/tmp/" +
                req.file.originalname;
              let output = `${__dirname}/tmp/playlistImg.jpg`.replace(
                /(\s+)/g,
                "\\$1"
              );
              await exec(
                `convert "${file}" -define jpeg:extent=190kb ${output}`
              );
              ...
              try {
                ...
                res.send({
                  message:
                    "Could not update playlist cover image. Try a different image.",
                  reload: false,
                });
                fs.unlinkSync(file);
                fs.unlinkSync(output);
